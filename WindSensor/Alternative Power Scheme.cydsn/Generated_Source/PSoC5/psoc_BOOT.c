...
static uint8 XDATA psoc_slReadBuf[psoc_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer: Host reads responses from it */
static uint8 XDATA psoc_slWriteBuf[psoc_BTLDR_SIZEOF_WRITE_BUFFER];
...
*
* Global variables:
*  psoc_slWriteBuf - used to store received command.
*  psoc_slReadBuf - used to store response.
...
{
    /* Set Write buffer */
    psoc_SlaveInitWriteBuf(psoc_slWriteBuf, psoc_BTLDR_SIZEOF_WRITE_BUFFER);

    /* Set Read buffer which has zero elements */
    psoc_SlaveInitReadBuf(psoc_slReadBuf, 0u);
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from the host */
        {
            ...
            if(0u != (psoc_slStatus & psoc_SSTAT_WR_CMPLT))
            {
                /* How many bytes the host has been written */
                byteCount = psoc_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) psoc_slWriteBuf,
                              psoc_MIN_UNT16(byteCount, size));
