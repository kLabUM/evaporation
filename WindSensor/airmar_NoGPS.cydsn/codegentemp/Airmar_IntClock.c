...
/*******************************************************************************
* Function Name: Airmar_IntClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Airmar_IntClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    Airmar_IntClock_CLKEN |= Airmar_IntClock_CLKEN_MASK;
	Airmar_IntClock_CLKSTBY |= Airmar_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Airmar_IntClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Airmar_IntClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Airmar_IntClock_CLKEN &= (uint8)(~Airmar_IntClock_CLKEN_MASK);
	Airmar_IntClock_CLKSTBY &= (uint8)(~Airmar_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Airmar_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Airmar_IntClock_StopBlock(void) 
{
    if ((Airmar_IntClock_CLKEN & Airmar_IntClock_CLKEN_MASK) != 0u)
    {
...
#if defined(Airmar_IntClock__CFG3)
        CLK_DIST_AMASK = Airmar_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Airmar_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Airmar_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Airmar_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Airmar_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Airmar_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Airmar_IntClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Airmar_IntClock_CLKSTBY &= (uint8)(~Airmar_IntClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Airmar_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Airmar_IntClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Airmar_IntClock_GetSourceRegister();
    uint16 oldDivider = Airmar_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Airmar_IntClock_CLKEN & Airmar_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Airmar_IntClock_DIV_PTR, clkDivider);
                Airmar_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Airmar_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Airmar_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Airmar_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Airmar_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Airmar_IntClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(Airmar_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Airmar_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Airmar_IntClock_SetModeRegister(uint8 modeBitMask) 
{
    Airmar_IntClock_MOD_SRC |= modeBitMask & (uint8)Airmar_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Airmar_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Airmar_IntClock_ClearModeRegister(uint8 modeBitMask) 
{
    Airmar_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Airmar_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Airmar_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Airmar_IntClock_GetModeRegister(void) 
{
    return Airmar_IntClock_MOD_SRC & (uint8)(Airmar_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Airmar_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Airmar_IntClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Airmar_IntClock_GetDividerRegister();
    uint8 oldSrc = Airmar_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Airmar_IntClock_MOD_SRC |= CYCLK_SSS;
        Airmar_IntClock_MOD_SRC =
            (Airmar_IntClock_MOD_SRC & (uint8)(~Airmar_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Airmar_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Airmar_IntClock_GetSourceRegister(void) 
{
    return Airmar_IntClock_MOD_SRC & Airmar_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Airmar_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Airmar_IntClock_SetPhaseRegister(uint8 clkPhase) 
{
    Airmar_IntClock_PHASE = clkPhase & Airmar_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Airmar_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Airmar_IntClock_GetPhaseRegister(void) 
{
    return Airmar_IntClock_PHASE & Airmar_IntClock_PHASE_MASK;
}
