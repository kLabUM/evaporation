...
int main()
{
    ...
    for(;;)
    {
		...
		scalerspeed = windsum / validpackets;
...
void getgpsdata()
{
	...
	char * startofgpsdata = strstr(SBDData, "$");
	int matches = sscanf(startofgpsdata, "$%*c%*c%*c%*c%*c,%f,%c,%f,%*c,%f,%*c,%*f,%*f,%d%*s", &time, &validornot, &northgps, &westgps, &date);
